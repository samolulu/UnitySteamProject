...
{
    float3 viewVector = mul(unity_CameraInvProjection, float4(screenPos * 2 - 1, 0, -1)).xyz;
...
float3 GetLighting(float3 rayPos, Texture2D additional, sampler sampler_additional, float additionalLightsCount)
{
    ...
    for (int x = 0; x < additionalLightsCount; x++)
    {
        ...
        float4 lightPosRange = SAMPLE_TEXTURE2D_LOD(additional, sampler_additional, float2(x / width, 0.15), 0);
        ...
        if (distance(lightPos, rayPos) < lightRange)
        {
            ...
            float4 lightVectorAngle = SAMPLE_TEXTURE2D_LOD(additional, sampler_additional, float2(x / width, 0.25), 0);
            ...
            if (isInCone > 0)
            {
                float3 lightCol = SAMPLE_TEXTURE2D_LOD(additional, sampler_additional, float2(x / width, 0.05), 0).xyz;
...
void RenderFog_float(float density, float anisotropy, float3 boxPos, float3 boxBounds, float steps, float stepSize, float depth, float4 screenPos, float lowResDist, float lowResMultiplier, float maxStepSize, float jitter, Texture2D additional, sampler sampler_additional, float additionalLightsCount, out float3 _color, out float3 _lightColor, out float3 _lightDir, out float _phase, out float _density)
{
    ...
    boxPos.xyz += _WorldSpaceCameraPos.xyz;
    _color = 0;
    _lightColor = 0;
    _lightDir = 0;
    _density = 0;
    ...
    //Intersection Ray
    float3 viewVector = mul(unity_CameraInvProjection, float4(screenPos.xy * 2 - 1, 0, -1)).xyz;
    float3 viewDir = mul(unity_CameraToWorld, float4(viewVector, 0)).xyz;
    float viewLength = length(viewDir);
    ...
    //Normalize Density
    float normalizedDensity = exp(-totalDensity);
