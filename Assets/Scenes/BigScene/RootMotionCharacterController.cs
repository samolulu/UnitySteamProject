...
public class RootMotionCharacterController : MonoBehaviour
{
	...
	void Update()
	{
		...
		// 处理速度控制
		bool isMoving = verticalInput > 0.01f;
		if (isMoving)
			currentSpeed = Mathf.MoveTowards(currentSpeed, maxSpeed * Mathf.Abs(verticalInput), acceleration * Time.deltaTime);
		else
			currentSpeed = Mathf.MoveTowards(currentSpeed, 0f, (deceleration + Mathf.Abs(verticalInput)) * Time.deltaTime);
